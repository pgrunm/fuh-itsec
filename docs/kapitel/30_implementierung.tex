...
Im Verzeichnis /etc/mysql erstellt man eine Skriptdatei namens mysql_backup.sh mit dem folgenden Inhalt:

#!/bin/bash
sudo mysqldump -u root messages > /etc/mysql/backup/backup_messages.sql

Dieses Skript verwendet den Befehl \enquote{mysqldump}, um eine Sicherungskopie der Datenbank \enquote{messages} zu erstellen. Die \verb+Backup-Datei+ wird im Verzeichnis /etc/mysql/backup/ mit dem Namen \enquote{backup_messages.sql} gespeichert.
