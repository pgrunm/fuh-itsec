...
Sollte es dennoch einem Angreifer gelingen, Zugriff auf den Server zu erlangen, sind dort noch zusätzlich der Auditd Service und OSSEC als \ac{IDS} installiert. Auditd protokolliert sämtliche Aktionen in der Log Datei \mint[]{shell}|/var/log/audit/audit.log| sodass sämtliche Aktionen nachvollzogen werden können.

Zusätzlich bietet OSSEC an, neben der Intrusion Detection, auch Dateien auf Änderungen zu Überwachen. Dies ermöglicht die Alarmierung eines Administrators, sofern Manipulationen festgestellt werden.
